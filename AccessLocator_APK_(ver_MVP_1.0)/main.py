...
class Interface(ScreenManager):
    ...
    def btn_continue(self):
        ...
        if not self.is_empty(self.user):
            if self.is_user():
                ...
                self.popup(text='Hibás felhasználó!', title='Nem létező user ID.')
        else:
            self.popup(text='Add meg a user ID-t!', title='A mező kitöltése kötelező.')
    ...
    def btn_search(self):
        ...
        if not self.is_empty(self.keywords):
            ...
            if not len(self.links_to_print) == 0:
                self.links_widget()
                self.current = 'results'
                self.transition.direction = 'left'
            else:
                self.popup(text='Nincs eredmény.')

        else:
            self.popup(text='Add meg a kulcsszavakat!', title='A mező kitöltése kötelező.')
